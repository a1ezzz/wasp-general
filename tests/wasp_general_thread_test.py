...
from wasp_general.thread import WCriticalSectionError, WAtomicCounter
...
class TestWAtomicCounter:
	...
	def test(self):
		...
		c = WAtomicCounter()
		assert(c.counter_value() == 0)

		c.increase_counter()
		assert(c.counter_value() == 1)

		c.increase_counter(delta=10)
		assert(c.counter_value() == 11)

		c = WAtomicCounter(5)
		assert(c.counter_value() == 5)

		c.increase_counter()
		assert(c.counter_value() == 6)
	...
	def test_multi_threading(self):
		...
		assert(c.counter_value() == (self.__threads__ * self.__repeats__))
