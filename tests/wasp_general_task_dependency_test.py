...
class TestWTaskDependencyRegistryStorage:
	...
	def test_task(self):
		...
		assert(registry_storage.started_task('task2') is None)
		registry_storage.start_task('task2')
		assert(isinstance(registry_storage.started_task('task2'), T2) is True)
		...
		assert(registry_storage.started_task('task6') is None)
		pytest.raises(RuntimeError, registry_storage.start_task, 'task6')
		registry_storage.start_task('task6', skip_unresolved=True)
		assert(isinstance(registry_storage.started_task('task6'), T6) is True)
